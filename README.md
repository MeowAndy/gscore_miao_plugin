...
- 抽卡与统计：`喵喵抽卡统计`、`喵喵角色持有率`、`喵喵深渊出场率`、`喵喵幽境危战出场率`、`喵喵深渊组队` 均输出图片卡。
- 角色图库：`喵喵雷电将军卡片`、`喵喵老婆 雷电将军`、`喵喵上传雷电将军照片 <URL>`、`喵喵雷电将军照片列表`、`喵喵原图`。
- Wiki 素材：`喵喵今日素材`、`喵喵明日材料`、`喵喵周一素材` 会聚合本地角色 Wiki 天赋材料并图片化输出。
...
- 资料与规划入口：星铁光锥图鉴、抽卡统计、统计图片与角色图库命令域已按 miao-plugin 风格接入。
...
当前版本为 **功能语义迁移（v0.15.19）**，重点迁移命令形态、UID 绑定、面板管理入口、配置管理、更新日志、基础权限控制、面板数据源、图片模板、角色别名、圣遗物/遗器评分、登录签到、抽卡统计、公开统计、角色图库与原神/星铁命令域。
