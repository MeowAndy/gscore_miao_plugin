...
@sv_feature.on_regex(r"^(原神|崩铁|星铁)?(?!(今日|今天|每日|明日|明天|周[1-7一二三四五六日天])(?:素材|材料|天赋)$)(?P<name>.+?)(?P<mode>天赋|技能|行迹|命座|命之座|星魂|资料|图鉴|材料|素材|养成)$", block=True)
...
@sv_feature.on_regex(
    r"^原神(?!(?:(?:米游社|mys)(?:全部面板更新|更新全部面板|获取游戏角色详情|更新面板|面板更新|刷新面板|面板刷新)|更新面板|刷新面板|面板刷新|全部面板更新|重载面板|删除面板|解绑UID|解绑uid|角色面板图|面板图|面板列表|面板角色列表|角色列表|面板|角色面板|圣遗物列表|遗物列表|圣遗物评分|遗物评分|伤害计算|伤害估算)(?:\s|$))(?P<name>.+?)\s*(?P<mode>面板|面版|详情|详细|圣遗物|遗器|伤害)\s*(?P<extra>.*?)(?P<uid>\d{9,10})?$",
    block=True,
)
...
@sv_feature.on_regex(
    r"^崩铁(?!(?:(?:米游社|mys)(?:全部面板更新|更新全部面板|获取游戏角色详情|更新面板|面板更新|刷新面板|面板刷新)|更新面板|刷新面板|面板刷新|全部面板更新|重载面板|删除面板|解绑UID|解绑uid|面板列表|面板角色列表|角色列表|面板角色|角色面板|面板|遗器列表|圣遗物列表|(?:面板|喵喵)?练度统计|角色持有率|角色命座统计|角色5命统计|抽卡记录|抽卡统计|抽奖记录|祈愿记录|祈愿统计|抽卡分析|角色池抽卡统计|光锥池抽卡统计|常驻池抽卡统计)(?:\s|$))(?!(?:.*(?:最强|最高分|第一|最高|最牛|最多|排名|排行|排行榜|排行版|榜))$)(?P<name>.+?)\s*(?P<mode>面板|面版|详情|详细|遗器|圣遗物|光锥|伤害)?\s*(?P<extra>.*?)(?P<uid>\d{9,10})?$",
    block=True,
)
