...
@sv_gacha.on_regex(r"^(?P<game>原神|崩铁|星铁)?(?P<pool>角色|武器|光锥|常驻|集录|up|UP)?池?(?P<cmd>抽卡记录|抽卡统计|抽奖记录|祈愿记录|祈愿统计|抽卡分析)\s*(?P<uid>\d{9,10})?$", block=True)
async def send_gacha_stat(bot: Bot, ev: Event):
    ...
    uid = await _uid_from_event(ev, data.get("uid") or text, game)
    if not uid:
