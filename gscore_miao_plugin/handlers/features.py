...
@sv_feature.on_regex(
    r"^崩铁(?!(?:(?:米游社|mys)(?:全部面板更新|更新全部面板|获取游戏角色详情|更新面板|面板更新)|更新面板|刷新面板|全部面板更新|重载面板|删除面板|解绑UID|解绑uid|面板列表|面板角色列表|角色列表|面板角色|角色面板|面板|遗器列表|圣遗物列表)(?:\s|$))(?P<name>.+?)\s*(?P<mode>面板|面版|详情|详细|遗器|圣遗物|光锥|伤害)?\s*(?P<uid>\d{9,10})?$",
    block=True,
)
