...
### Added
- 新增 `抽卡记录/抽卡统计/祈愿统计` 图片卡，兼容 miao-plugin 本地 `gachaJson` / `srJson` 抽卡 JSON。
- 新增 `角色持有率`、`角色命座统计`、`深渊出场率`、`幽境危战出场率`、`深渊组队` 等统计图片入口。
- 新增角色卡片/写真、老婆/老公/女朋友/男朋友/女儿/儿子互动、原图入口。
- 新增自定义角色照片/写真/面板图 URL 上传、列表与删除命令。
- 新增 `今日素材`、`明日材料`、`周X素材` 图片卡，以及 `绝区零日历` 占位入口。
- 新增 `喵喵api`、`喵喵更新`、`喵喵更新图像` 管理状态卡。

## [0.15.18] - 2026-04-26

### Added
- `喵喵原神设置` / `喵喵崩铁设置` 改为图片设置卡片，展示 UID、面板源、开关状态、历史 UID 与常用命令。
- `喵喵登录` 成功后改为图片卡片，展示掩码 Cookie、原神/崩铁角色与后续可用命令。
- `米游社更新面板` 新增 `刷新面板` / `面板刷新` 同义词，兼容 `MM原神mys刷新面板` 等写法。

### Changed
- 米游社/普通刷新成功后的面板数据会写入 latest 缓存，后续自动面板查询、角色面板与伤害计算优先使用刚刷新的数据。
- 星铁米游社面板源接入 `avatarInfo`，并补齐 `equip`、`ornaments` 等字段兼容。

### Fixed
- 修复 `喵喵原神设置uid 切换` / `喵喵崩铁设置uid 切换` 未填写目标 UID 时误解绑的问题。
- 修复 `雷神伤害 敌人等级90 抗性10 万班 钟离` 等命令中 `敌人` 残留导致角色名匹配失败的问题。

## [0.15.17] - 2026-04-26

### Changed
- 面板渲染扩展原神/崩铁角色立绘与头像素材候选路径，数据源名称统一显示为 Miao-Api、Enka-Api、米游社、Mihomo、Avocado、EnkaHSR 等友好名称。
- 圣遗物/遗器评分补齐部位满分、套装字段兼容、部位字段兼容；崩铁 6 件遗器主词条计分更贴近 miao-plugin 评分体验。
- 面板数据源查询失败提示会展示已尝试源、关键错误与切换 `auto` 建议，减少只看到“查询失败”的情况。
- UID 管理新增原神/崩铁 UID 历史列表与切换入口，米游社登录会同步保存原神/崩铁 UID 历史。
- 更新面板、米游社更新面板、删除面板会清理本地面板缓存并在回退文本中展示清理数量。

### Added
- 新增 `喵喵崩铁设置` / `喵喵崩铁设置uid` / `喵喵崩铁设置面板服务`，可直接管理星铁 UID 与面板源。
- WebUI 面板服务选项补齐 Mihomo、Avocado、EnkaHSR，便于星铁面板源配置。

## [0.15.16] - 2026-04-26

### Changed
- 伤害估算补入敌人等级、防御区、抗性区、反应/击破、组队 Buff、武器/光锥与套装/遗器收益折算。
- 新增/增强原神胡桃、那维莱特、雷电将军、夜兰、芙宁娜、阿蕾奇诺、玛薇卡等角色模板。
- 新增/增强崩铁黄泉、流萤、镜流、飞霄、砂金、饮月、波提欧等角色模板。
- 角色自然伤害命令支持追加 `敌人等级90`、`抗性10`、`蒸发`、`万班`、`夜芙`、`钟离`、`阮梅`、`同谐主`、`花火`、`知更鸟`、`超击破` 等参数。
- 伤害估算图片增加敌人、防御/抗性、反应/击破、组队 Buff 与模板说明展示。
...
- 单件遗器分数与强化等级改为左右分列显示，减少主词条区域拥挤。
- 遗器评分详情图副词条评分固定在每列正右侧，不再显示到下方。

## [0.15.4] - 2026-04-26

### Changed
- 优化星铁遗器卡片排版，副词条数值与评分分列显示，评分使用橙色突出显示。
- 遗器名称改为按宽度自动缩放/分行，减少省略号截断。
- 单角色面板与遗器评分详情图同步增高，避免内容拥挤或被裁切。
...
- 面板列表底部创建信息改为 `Created By Miao-Plugin & MiHoYoUID 当前版本 By MeowAndy`，版本号自动读取插件版本。
- `喵喵更新日志` 改为使用转发节点形式发送。
